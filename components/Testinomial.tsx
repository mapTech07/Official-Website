...
import business from '../public/team_img/Man_Bahadur_Roka.jpg';
import ceo from '../public/team_img/Anil_Nhemhafuki.jpg';
import cto from '../public/team_img/Pawan_Bhattarai.jpg';
...
const Testimonials = () => {
  ...
  return (
    <section className="flex flex-col items-center px-6 py-10 w-full bg-gray-100">
      <h2 className="text-3xl font-semibold text-gray-800 mb-6">What Our Leaders Say</h2>
      ...
      >
        ...
        <div key={index} className="flex flex-col items-center transition-opacity duration-500">
          ...
          <p className="text-gray-700 italic text-lg mb-4">{testimonials[index].text}</p>
          <div className="mt-4">
            <h4 className="text-xl font-semibold text-gray-900">{testimonials[index].name}</h4>
            <p className="font-medium text-gray-600">{testimonials[index].role}</p>
          </div>
        </div>
      ...
      <div className="hidden">
        {testimonials.map((item, i) => (
          <Image key={i} src={item.image} alt={item.name} width={1} height={1} priority />
        ))}
      </div>
    </section>
  );
